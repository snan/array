...
class CsvTest : APLTest() {
    ...
    fun floatCells() = runBlockingCompat<Unit> {
        ...
        val result = readCsv(input)
        assertDimension(dimensionsOfSize(2, 3), result)
        assertDoubleWithRange(Pair(1.19999, 1.20001), result.valueAt(0))
        assertDoubleWithRange(Pair(0.29999, 0.30001), result.valueAt(1))
        assertDoubleWithRange(Pair(3.99999, 4.00001), result.valueAt(2))
        assertString(".", result.valueAt(3))
